...
export async function webhookLiveClassPayment(req, res) {
  ...
  try {
    ...
    if (["refund.created", "refund.processed"].includes(event.event)) { const refund = event.payload?.refund?.entity; const paymentId = refund?.payment_id; if (typeof paymentId !== "string") return res.status(400).json({ error: "Refund payment is missing." }); const eventId = typeof req.headers["x-razorpay-event-id"] === "string" ? req.headers["x-razorpay-event-id"] : `${event.event}:${refund.id ?? paymentId}`; await finalizeRefund(getSupabaseAdmin(), paymentId, eventId, { event: event.event, refund_id: refund.id, payment_id: paymentId }); return res.status(200).json({ received: true }); }
