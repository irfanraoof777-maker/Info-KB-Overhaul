...
-- Fix SQLSTATE 42702 in the service-only launch-configuration RPC. The
-- RETURNS TABLE output variable rental_id collided with the prior
-- ON CONFLICT (rental_id) inference expression.
...
BEGIN
  ...
  -- This exact parent-row lock serializes configuration saves for one rental.
