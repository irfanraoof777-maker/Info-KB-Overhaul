...
-- The existing RPC is idempotent: a repeat claim returns the prior rental.
-- Add an explicit signal so the server can notify only for a newly inserted row.
DROP FUNCTION public.claim_free_lab_rental(uuid, uuid);

CREATE FUNCTION public.claim_free_lab_rental(
  p_student_id uuid,
  p_lab_id uuid
)
RETURNS TABLE (
...
BEGIN
  ...
  IF existing_rental.id IS NOT NULL THEN
    RETURN QUERY SELECT
      ...
      existing_rental.updated_at, false;
    RETURN;
  END IF;
  ...
  IF claimed_rental.id IS NULL THEN
    ...
    RETURN QUERY SELECT
      ...
      claimed_rental.updated_at, false;
    RETURN;
  END IF;
  ...
  RETURN QUERY SELECT
    ...
    claimed_rental.cancelled_at,
    claimed_rental.created_at,
    claimed_rental.updated_at, true;
END;
$function$;
